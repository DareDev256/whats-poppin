...
src/
  ...
  game.js        — Grid engine, 7 Phaser scenes (incl. StatsScene), processMatches() pipeline (see below, depth-capped at 20), hint system, fever meter, TEXT_PRESETS + textStyle() typography system, achievement system (ACHIEVEMENTS + Achievements helper), VALID_MODES allowlist for scene data sanitization, per-frame render subsystems (renderPowerUpOverlays, renderFeverMeter, renderHintGlow), shared helpers (scanRuns, createButton, createToolbarBtn, safeDiv, safeScore, etc.)
